...
def sample_prim_bounds(ex, sp_bounds, rhs_bounds):
    ...
    width_val = multi_norm_sample(
        ex,
        ex.get_prim_bounds_dim_pdist(w_max),
        MO,
        w_max,
        'scale',
    )
    ...
    x_min = sp_bounds[3] - sp_bounds[0] + width_val
    x_max = sp_bounds[3] + sp_bounds[0] - width_val

    y_min = sp_bounds[4] - sp_bounds[1] + height_val
    y_max = sp_bounds[4] + sp_bounds[1] - height_val

    z_min = sp_bounds[5] - sp_bounds[2] + depth_val
    z_max = sp_bounds[5] + sp_bounds[2] - depth_val
    ...
    if rhs_bounds is not None:
        ...
        z_min = max(rhs_bounds[4].item() + depth_val + MO, z_min)
        z_max = min(rhs_bounds[5].item() - depth_val - MO, z_max)
    ...
    xpos_val = multi_norm_sample(
        ex,
        ex.get_prim_bounds_pos_pdist(cxpos, width_val),
        x_min,
        x_max,
        'move',
    )
    ...
    ypos_val = multi_norm_sample(
        ex,
        ex.get_prim_bounds_pos_pdist(cypos, height_val),
        y_min,
        y_max,
        'move',
    )

    zpos_val = multi_norm_sample(
        ex,
        ex.get_prim_bounds_pos_pdist(czpos, depth_val),
        z_min,
        z_max,
        'move',
    )

    return (width_val, height_val, depth_val, xpos_val, ypos_val, zpos_val)
...
def resolve_params(ex, ho, nparams, const_type, prev_params):
    ...
    if const_type == 'all' or \
       nparams == prev_params or (
           const_type == 'first' and
           nparams[1] == prev_params[1] and
           nparams[2] == prev_params[2]
       ) or (
           const_type == 'mid' and
           nparams[0] == prev_params[0] and
           nparams[2] == prev_params[2]
       ) or (
           const_type == 'last' and
           nparams[1] == prev_params[1] and
    ...
    elif const_type=='last':
        D[2] = correct_for(ex, ho,prev_params[2],nparams[2])
    else:
        assert False
...
def sample_rot_params(ex):

    BR = 'rflt_1'
    assert ex.T2F('rot', BR) == 0.0
    
    ROPTS = [r for r in ex.FLT_INFO['rot'].keys() if r != BR]
    
    sparams = [random.choice(ROPTS) for _ in range(3)]

    rinds = lu.sample_dist(RINDS_DIST)

    rparams = [BR, BR, BR]

    for ri in rinds:
        rparams[ri] = sparams[ri]
...
class Prim:
    def __init__(self, ex, bounds, par):
        ...
        if par is None:
            width, height, depth, xpos, ypos, zpos = bounds
            self.scale_params = (width, height, depth)
            self.move_params = (xpos, ypos, zpos)
        else:
            prim_info = sample_prim_bounds(ex, bounds, par.rhs_bounds)
            width, height, depth, xpos, ypos, zpos = prim_info
            self.scale_params = (width, height, depth)
            self.move_params = (xpos, ypos, zpos)

        if random.random() < ROT_CHANCE:            
            self.rot_params = sample_rot_params(ex)
        else:
            self.rot_params = None
    ...
    def get_area(self):
        return self.area.view(1,6)
    
    def get_bbox(self):
        xpos, ypos, zpos = self.move_params
        width, height, depth = self.scale_params
        return torch.tensor([
            xpos - width,
            xpos + width,
            ypos - height,
...
def filter_valid(ex, sub_progs, prev_occ):
    ...
    while len(valid_sub_progs) > 0:
        ...
        if uniq_occ[amin] >= ex.MIN_DIFF_VOXELS:
