...
def print_results(
    ...
    for info in LOG_INFO:
        ...
        if label is not None:
            res += f"    {label.split('_')[0]}_{name} : {round(_res, 4)}\n"
            res_dict[label+name] = round(_res, 4)
        else:
            res += f"    {name} : {round(_res, 4)}\n"
            res_dict[name] = round(_res, 4)
    ...
    for mname in mnames:
        ...
        for sname in snames:
            ...
            if len(epochs) == len(vals):
                ...
                log_print(f"Bad plot mismatch for {sname} ({len(epochs)} | {len(vals)})", args)
